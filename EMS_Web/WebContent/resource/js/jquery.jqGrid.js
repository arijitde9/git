...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addCell = function(rowId,cell,pos,irow, srvr, rdata) {
			...
			return "<td role=\"gridcell\" "+prp+"><div class='gridcellclass'>"+v+"</div></td>";
		...
		addRowNum = function (pos,irow,pG,rN) {
			...
			return "<td role=\"gridcell\" class=\"ui-state-default jqgrid-rownum\" "+prp+"><div class='gridcellclass'>"+v+"</div></td>";
			//return "<td role=\"gridcell\" class=\"ui-state-default jqgrid-rownum\" "+prp+">"+v+"</div></td>";
		},
		...
		constructTr = function(id, hide, altClass, rd, cur, selected) {
			var tabindex = '-1', restAttr = '', attrName, style = hide ? 'display:none;' : '',
				classes = 'ui-widget-content jqgrow ui-row-' + ts.p.direction + (altClass ? ' ' + altClass : '') + (selected ? ' ui-edited-state-highlight' : ''),
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pgbuttons===true) {
					if(cp<=0) {cp = last = 0;}
					if(cp===1 || cp === 0) {
						$("#first"+tspg+", #prev"+tspg).addClass('ui-state-disabled').removeClass('ui-edited-state-hover');
						if(ts.p.toppager) { $("#first_t"+tspg_t+", #prev_t"+tspg_t).addClass('ui-state-disabled').removeClass('ui-edited-state-hover'); }
					} else {
						$("#first"+tspg+", #prev"+tspg).removeClass('ui-state-disabled');
						if(ts.p.toppager) { $("#first_t"+tspg_t+", #prev_t"+tspg_t).removeClass('ui-state-disabled'); }
					}
					if(cp===last || cp === 0) {
						$("#next"+tspg+", #last"+tspg).addClass('ui-state-disabled').removeClass('ui-edited-state-hover');
						if(ts.p.toppager) { $("#next_t"+tspg_t+", #last_t"+tspg_t).addClass('ui-state-disabled').removeClass('ui-edited-state-hover'); }
					} else {
		...
		setPager = function (pgid, tp){
			...
			$(".ui-pg-button","#"+pgcnt).hover(function(){
				if($(this).hasClass('ui-state-disabled')) {
					...
					$(this).addClass('ui-edited-state-hover');
					this.style.cursor='pointer';
				}
			},function() {
				if(!$(this).hasClass('ui-state-disabled')) {
					$(this).removeClass('ui-edited-state-hover');
					this.style.cursor= "default";
				}
			});
		...
		imgs = null;
		$(this).append(thead);
		$("thead tr:first th",this).hover(function(){$(this).addClass('ui-edited-state-hover');},function(){$(this).removeClass('ui-edited-state-hover');});
		if(this.p.multiselect) {
			var emp=[], chk;
					...
					$(ts.rows).each(function(i) {
						if (i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled')){
								...
								$(this).addClass("ui-edited-state-highlight").attr("aria-selected","true");  
								ts.p.selarrrow.push(this.id);
								ts.p.selrow = this.id;
								if(froz) {
									$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id), ts.grid.fbDiv )[ts.p.useProp ? 'prop': 'attr']("checked",true);
									$("#"+$.jgrid.jqID(this.id), ts.grid.fbDiv).addClass("ui-edited-state-highlight");
								}
							}
						}
					});
					...
					$(ts.rows).each(function(i) {
						if(i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass('ui-state-disabled')){
								...
								$(this).removeClass("ui-edited-state-highlight").attr("aria-selected","false");
								emp.push(this.id);
								if(froz) {
									$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id), ts.grid.fbDiv )[ts.p.useProp ? 'prop': 'attr']("checked",false);
									$("#"+$.jgrid.jqID(this.id), ts.grid.fbDiv).removeClass("ui-edited-state-highlight");
								}
							}
						}
					});
		...
		$(ts).bind('mouseover',function(e) {
			...
			if($(ptr).attr("class") !== "ui-subgrid") {
				$(ptr).addClass("ui-edited-state-hover");
			}
		}).bind('mouseout',function(e) {
			ptr = $(e.target).closest("tr.jqgrow");
			$(ptr).removeClass("ui-edited-state-hover");
		});
		}
		var ri,ci, tdHtml;
					...
					if(ts.p.multiselect && ts.p.multiboxonly) {
						if(scb){$(ts).jqGrid("setSelection",ri,true,e);}
							...
							$(ts.p.selarrrow).each(function(i,n){
								...
								$(ind).removeClass("ui-edited-state-highlight");
								$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(n))[ts.p.useProp ? 'prop': 'attr']("checked", false);
								if(frz) {
									$("#"+$.jgrid.jqID(n), "#"+$.jgrid.jqID(frz)).removeClass("ui-edited-state-highlight");
		...
		var arf = ts.p.hidegrid===true ? $("<a role='link' href='javascript:void(0)'/>").addClass('ui-jqgrid-titlebar-close HeaderButton').hover(
			function(){ arf.addClass('ui-edited-state-hover');},
			function() {arf.removeClass('ui-edited-state-hover');})
		...
		return this.each(function(){
			...
			if(!$t.p.multiselect) {	
				if(pt.className !== "ui-subgrid") {
					if( $t.p.selrow !== pt.id) {
						$($t.rows.namedItem($t.p.selrow)).removeClass("ui-edited-state-highlight").attr({"aria-selected":"false", "tabindex" : "-1"});
						$(pt).addClass("ui-edited-state-highlight").attr({"aria-selected":"true", "tabindex" : "0"});//.focus();
						if(fid) {
							$("#"+$.jgrid.jqID($t.p.selrow), "#"+$.jgrid.jqID(fid)).removeClass("ui-edited-state-highlight");
							$("#"+$.jgrid.jqID(selection), "#"+$.jgrid.jqID(fid)).addClass("ui-edited-state-highlight");
						}
						stat = true;
					} else {
						stat = false;
				...
				if (  ia === -1 ){
					if(pt.className !== "ui-subgrid") { $(pt).addClass("ui-edited-state-highlight").attr("aria-selected","true");}
					stat = true;
					$t.p.selarrrow.push($t.p.selrow);
				} else {
					if(pt.className !== "ui-subgrid") { $(pt).removeClass("ui-edited-state-highlight").attr("aria-selected","false");}
					stat = false;
				...
				if(fid) {
					if(ia === -1) {
						...
						$("#"+$.jgrid.jqID(selection), "#"+$.jgrid.jqID(fid)).removeClass("ui-edited-state-highlight");
					}
	...
	resetSelection : function( rowid ){
		return this.each(function(){
			...
			if(rowid !== undefined ) {
				...
				$("#"+$.jgrid.jqID(t.p.id)+" tbody:first tr#"+$.jgrid.jqID(sr)).removeClass("ui-edited-state-highlight").attr("aria-selected","false");
				if (fid) { $("#"+$.jgrid.jqID(sr), "#"+$.jgrid.jqID(fid)).removeClass("ui-edited-state-highlight"); }
				if(t.p.multiselect) {
				...
				if(t.p.selrow) {
					$("#"+$.jgrid.jqID(t.p.id)+" tbody:first tr#"+$.jgrid.jqID(t.p.selrow)).removeClass("ui-edited-state-highlight").attr("aria-selected","false");
					if(fid) { $("#"+$.jgrid.jqID(t.p.selrow), "#"+$.jgrid.jqID(fid)).removeClass("ui-edited-state-highlight"); }
					t.p.selrow = null;
				}
			} else {
				$(t.p.selarrrow).each(function(i,n){
					ind = t.rows.namedItem(n);
					$(ind).removeClass("ui-edited-state-highlight").attr("aria-selected","false");
					$("#jqg_"+$.jgrid.jqID(t.p.id)+"_"+$.jgrid.jqID(n))[t.p.useProp ? 'prop': 'attr']("checked",false);
					if(fid) { 
						$("#"+$.jgrid.jqID(n), "#"+$.jgrid.jqID(fid)).removeClass("ui-edited-state-highlight"); 
			...
			if(t.p.cellEdit === true) {
				if(parseInt(t.p.iCol,10)>=0  && parseInt(t.p.iRow,10)>=0) {
					$("td:eq("+t.p.iCol+")",t.rows[t.p.iRow]).removeClass("edit-cell ui-edited-state-highlight");
					$(t.rows[t.p.iRow]).removeClass("selected-row ui-edited-state-hover");
				}
			}
			t.p.savedRow = [];
		});
		...
		return this.each(function(){
			...
			buildRuleMenu = function( elem, left, top ){
				...
				for ( i = 0 ; i < options.sopt.length; i++) {
					...
					if(ina !== -1) {
						selclass = selected === p.odata[ina].oper ? "ui-edited-state-highlight" : "";
				...
				$("#sopt_menu > li > a").hover(
					function(){ $(this).addClass("ui-edited-state-hover"); },
					function(){ $(this).removeClass("ui-edited-state-hover"); }
				).click(function( e ){
				...
				$($t).bind('jqGridAfterGridComplete.setFrozenColumns', function () {
					...
					if($t.p.hoverrows === true) {
						$("tr.jqgrow", btbl).hover(
							function(){ $(this).addClass("ui-edited-state-hover"); $("#"+$.jgrid.jqID(this.id), "#"+$.jgrid.jqID($t.p.id)).addClass("ui-edited-state-hover"); },
							function(){ $(this).removeClass("ui-edited-state-hover"); $("#"+$.jgrid.jqID(this.id), "#"+$.jgrid.jqID($t.p.id)).removeClass("ui-edited-state-hover"); }
						);
						$("tr.jqgrow", "#"+$.jgrid.jqID($t.p.id)).hover(
							function(){ $(this).addClass("ui-edited-state-hover"); $("#"+$.jgrid.jqID(this.id), "#"+$.jgrid.jqID($t.p.id)+"_frozen").addClass("ui-edited-state-hover");},
							function(){ $(this).removeClass("ui-edited-state-hover"); $("#"+$.jgrid.jqID(this.id), "#"+$.jgrid.jqID($t.p.id)+"_frozen").removeClass("ui-edited-state-hover"); }
						);
					}
					btbl=null;
				});
...
"use strict";	
	...
	$.fn.fmatter.actions = function(cellval,opts) {
		...
		if(op.editformbutton){
			ocl = "id='jEditButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'formedit'); onmouseover=jQuery(this).addClass('ui-edited-state-hover'); onmouseout=jQuery(this).removeClass('ui-edited-state-hover'); ";
			str += "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='ui-icon ui-icon-pencil'></span></div>";
		} else if(op.editbutton){
			ocl = "id='jEditButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'edit'); onmouseover=jQuery(this).addClass('ui-edited-state-hover'); onmouseout=jQuery(this).removeClass('ui-edited-state-hover') ";
			str += "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='ui-icon ui-icon-pencil'></span></div>";
		}
		if(op.delbutton) {
			ocl = "id='jDeleteButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'del'); onmouseover=jQuery(this).addClass('ui-edited-state-hover'); onmouseout=jQuery(this).removeClass('ui-edited-state-hover'); ";
			str += "<div title='"+$.jgrid.nav.deltitle+"' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' "+ocl+"><span class='ui-icon ui-icon-trash'></span></div>";
		}
		ocl = "id='jSaveButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'save'); onmouseover=jQuery(this).addClass('ui-edited-state-hover'); onmouseout=jQuery(this).removeClass('ui-edited-state-hover'); ";
		str += "<div title='"+$.jgrid.edit.bSubmit+"' style='float:left;display:none' class='ui-pg-div ui-inline-save' "+ocl+"><span class='ui-icon ui-icon-disk'></span></div>";
		ocl = "id='jCancelButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'cancel'); onmouseover=jQuery(this).addClass('ui-edited-state-hover'); onmouseout=jQuery(this).removeClass('ui-edited-state-hover'); ";
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		.hover(function(){ahr.addClass('ui-edited-state-hover');},
			function(){ahr.removeClass('ui-edited-state-hover');})
	...
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$(cc).addClass("edit-cell ui-edited-state-highlight");
				$($t.rows[iRow]).addClass("selected-row ui-edited-state-hover");
				try {
			...
			var sortable_opts = {
				...
				"placeholder": {
					element: function(item) {
						...
						.addClass(item[0].className+" ui-sortable-placeholder ui-edited-state-highlight")
	...
	gridDnD : function(opts) {
		...
		opts = $.extend({
			"drag" : function (opts) {
				return $.extend({
					start : function (ev, ui) {
						...
						ui.helper.addClass("ui-edited-state-highlight");
			...
			"drop_opts" : {
				...
				"hoverClass": "ui-edited-state-hover"
			},
			"drag_opts" : {
				"revert": "invalid",
				"helper": "clone",
